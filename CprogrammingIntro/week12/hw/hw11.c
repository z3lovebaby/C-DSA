...
#include <time.h>
#define maxLeng 25
void Update(float *a,float x,int i){
  int j;
  for (j=1;j<=3;j++){
    *(a+4*i+j)*=x;
  }
...
void arena(int i,int j,float *a,char name[][maxLeng]){
  int turn;
  float h1,h2;
  h1=*(a+4*(i-1)+2);
  h2=*(a+4*(j-1)+2);
  ...
  printf("Speed of %s: %.2f\n",name[j-1],*(a+4*(j-1)+1));
  if (*(a+4*(i-1)+1)>*(a+4*(j-1)+1)){
    turn = 0;

  }else if (*(a+4*(i-1)+1)<*(a+4*(j-1)+1)){
    turn = 1;
  }else {
  ...
  do{
    ...
    printf("%25s VS %s \n",name[i-1],name[j-1]);
    printf("_________________________________________________\n");
    printf("SPEED:\t%-20.2f\t",*(a+4*(i-1)+1));
    // printf("\t\t");
    printf("\t%-6.2f:SPEED\t",*(a+4*(j-1)+1));
    printf("\n");
    //
    ...
    printf("\n");
    //
    printf("POWER:\t%-20.2f\t",*(a+4*(i-1)+3));
    printf("\t%-6.2f:POWER",*(a+4*(j-1)+3));
    printf("\n");
    //
    printf("_________________________________________________\n");
    if (turn ==0){
...
//
void pri(float *a,char name[][maxLeng]){
  printf("+=============================================+ \n");
  printf("|                    |       |        |       |\n");
  ...
  printf("|====================|=======|========|=======|\n");
  printf("|                    |       |        |       |\n");
  printf("|\t%-13s|%-7.2f| %-7.2f| %-6.2f|\n",name[1],*(a+4*1+1),*(a+4*1+2),*(a+4*1+3));
  printf("|====================|=======|========|=======|\n");
  printf("|                    |       |        |       |\n");
  printf("|\t%-13s|%-7.2f| %-7.2f| %-6.2f|\n",name[2],*(a+4*2+1),*(a+4*2+2),*(a+4*2+3));
  printf("+====================|=======|========|=======+\n");
  printf("\t\t=-Press ENTER to Continue-=");
  while( getchar() != '\n' );
}
